...
/// Create / view / edit form for a single employee (spec 035 US5 —
/// converted from the pushed `/employees/new` / `/employees/:employeeId`
/// routes to the shared record panel, `showRecordSheet`). [employeeId] is
/// `null` in create mode. See `label_form.dart` for the base pattern
/// (spec 035 T029).
///
/// Note: unlike the T037 task description assumed, this form has no
/// `CatalogEntityPicker` — `taxpayerId` is a plain string field, not a
/// relation to Taxpayer Recipients (`employee_form_controller.dart` has no
/// picker-backed field at all). FR-035 (pickers keep working inside the
/// panel) therefore has nothing to verify for this entity.
class EmployeeForm extends ConsumerStatefulWidget {
  const EmployeeForm({super.key, this.employeeId, this.forceReadOnly = false});
  ...
  @override
  ConsumerState<EmployeeForm> createState() => EmployeeFormPanelState();
}

/// Public so a caller can address it via `GlobalKey<EmployeeFormPanelState>`
/// and query [isDirty] from `showRecordSheet`'s `isDirty` callback.
class EmployeeFormPanelState extends ConsumerState<EmployeeForm> {
  bool get _isEdit => widget.employeeId != null;

  late bool _readOnlyOverride = widget.forceReadOnly;

  /// Snapshot of [EmployeeFormState] captured once loading actually
  /// completes — awaited directly in [initState], not inferred from a
  /// `build()`-timing flag (spec 035 T028).
  EmployeeFormState? _snapshot;
  ...
  void initState() {
    ...
    if (_isEdit) {
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        ...
        if (mounted) {
          setState(() => _snapshot = ref.read(employeeFormControllerProvider));
        }
      });
    } else {
      _snapshot = ref.read(employeeFormControllerProvider);
    }
  }

  bool isDirty() {
    final current = ref.read(employeeFormControllerProvider);

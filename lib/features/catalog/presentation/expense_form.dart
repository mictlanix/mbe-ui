...
/// Create / view / edit form for a single expense (spec 035 US5 — converted
/// from the pushed `/expenses/new` / `/expenses/:expenseId` routes to the shared
/// record panel, `showRecordSheet`). [expenseId] is `null` in create mode.
///
/// Hosted by `ExpensesListScreen` inside `showRecordSheet`, not pushed as its
/// own route — the panel's own header supplies the "New/View/Edit Expense"
...
class ExpenseForm extends ConsumerStatefulWidget {
  const ExpenseForm({super.key, this.expenseId, this.forceReadOnly = false});
  ...
  @override
  ConsumerState<ExpenseForm> createState() => ExpenseFormPanelState();
}

/// Public so a caller can address it via `GlobalKey<ExpenseFormPanelState>` and
...
class ExpenseFormPanelState extends ConsumerState<ExpenseForm> {
  ...
  void initState() {
    ...
    if (_isEdit) {
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        ...
        if (mounted) setState(() => _snapshot = ref.read(expenseFormControllerProvider));
      });
    } else {
      // Create mode never loads — the pristine baseline is available
      // synchronously, from the controller's own default state.
      _snapshot = ref.read(expenseFormControllerProvider);
    }
  }
  ...
  bool isDirty() {
    final current = ref.read(expenseFormControllerProvider);

...
- [X] T019 [US2] Arrange the boolean attribute switches and the Precios section into a two-column band (switches left, prices right) on non-compact viewports, stacking on compact, via a `_SwitchesPricesBand` widget in `lib/features/catalog/presentation/product_detail_screen.dart` (FR-017); add wide/compact widget tests. Switches occupy the full width when a product has no price list.
- [X] T020 [US2] Add a `maxColumns` parameter to `ResponsiveFormGrid` (`lib/core/widgets/responsive_form_grid.dart`, default 3) and cap the product form at two columns even on the large tier via `maxColumns: 2` (FR-009); update the wide-viewport test to assert the unit field wraps below Code (two columns, not three).
- [X] T021 [US2] Bracket the attributes/prices band with Material 3 `Divider`s (keys `attributes_divider_top`/`attributes_divider_bottom`) before and after it in `product_detail_screen.dart` (FR-018); add a widget test asserting the dividers straddle the switches.

...
## Pre-Implementation Note (added 2026-07-05, post-plan)

A re-check of the generated API client and sibling `mbe-api` repo before task generation found that mbe-api independently extracted per-product pricing into its own service (`mbe-api@e4b50c5`) and mbe-ui's generated client was already regenerated against it (`2a9eddc`). `ProductResponse.prices` no longer exists, so **`dart analyze lib` currently fails** on `product.dart:86` (`Product.fromResponse` still reads `response.prices`). This is exactly the removal FR-012 already called for, just arriving from upstream first — it does not change scope, but it does mean the `Product`/`ProductPrice` entity cleanup (data-model.md's `prices` deltas) is a **build-fix prerequisite**, not an optional cleanup step, and `/speckit-tasks` should sequence it first (or fold it into US3's first task) rather than treat it as normal-priority polish. No other drift was found: SKU and supplier fields/semantics on the generated `ProductCreate`/`ProductUpdate`/`ProductResponse` are unchanged from what research.md/data-model.md already describe, and mbe-api's hard-delete (`delete_product`) still deletes a product's prices before deleting the product itself (now via `product_price_service.delete_for_product`), so contracts/product-repository.md's hard-delete contract is unaffected.

...
**Supplier (FR-011) — Decision**: The supplier picker already assigns/changes and persists (`supplierSelected` → `create`/`update` send `supplier`). Keep it. **Clearing is deferred**: mbe-api's update service does `if data.supplier is not None: product.supplier = data.supplier`, so a null supplier is treated as "leave unchanged" — the client cannot express "clear." Ship assign/change; record supplier-clear as a backend dependency (Complexity Tracking). Do not wire a clear affordance that silently no-ops.

**Prices (FR-012) — Decision**: Remove the price sub-panel from the form (`_SwitchesPricesBand`'s `prices` argument and the `formState.prices` rendering). Keep `Product.prices`/`ProductPrice` mapping intact for now — mbe-api still returns `prices`, and removing it from the API + regenerating the client is out of scope (spec Assumption). Optionally stop populating `ProductFormState.prices` since nothing reads it. Net effect: no price UI anywhere on the form.

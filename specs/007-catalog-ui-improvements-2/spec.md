...
- Removing the price-list section is primarily a client-side UI change to mbe-ui. **Update (2026-07-05)**: mbe-api has since removed `prices` from its product response independently (extracting per-product pricing into its own management service) and mbe-ui's generated API client has already been regenerated against that change — so this requirement is no longer purely additive UI cleanup; it also closes a pre-existing compile break in `Product.fromResponse` caused by that upstream removal. No mbe-api work is needed from this feature either way.

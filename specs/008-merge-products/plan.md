...
| III. Contract-Driven API | ✅ Pass (0 dependencies) | Uses the already-generated `mergeProductsApiV1ProductsMergePost` + `ProductMergeRequest`; product search reuses the generated `listProductsApiV1ProductsGet`. No regeneration, no hand-written DTOs. Errors mapped through the shared `mapDioException` → `AppError` and surfaced via the shared `error_banner`. |
...
| VI. Desktop/Web-First, Compact-Ready Layout | ✅ Pass | Not a list/table screen, so the row-action/table clauses don't apply. The two pickers are laid out in a width-constrained, centered column (reusing `responsive_form_grid` or a max-width container) so single fields never stretch edge-to-edge on wide displays. Breakpoints come from `core/`. |
...
*No constitution violations — table intentionally empty.* The one notable choice (route gated on Create, §IV design note) tightens rather than relaxes the default and is recorded in the Constitution Check and `contracts/routes.md`, not here.

## Phase 0 — Research

See [research.md](./research.md). All Technical Context items are resolved; no `NEEDS CLARIFICATION` remain. Key decisions: reuse `ProductRepository.list` (with `deactivated: null`) as the picker search source; extend `CatalogEntityPicker` with optional photo/subtitle option rendering rather than forking a new widget; gate the merge route on Create; and display name/code/model/brand (not SKU) in suggestion rows because the list projection has no `sku` field (SKU remains searchable server-side).

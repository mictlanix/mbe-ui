...
**Cross-repo dependency (resolved)**: FR-003 requires SKU in suggestion rows, but `ProductListItem` (the products-list projection both pickers reuse) initially didn't carry `sku`. Per this project's repo boundary — mbe-ui does not modify mbe-api directly — this was filed as [mictlanix/mbe-api#76](https://github.com/mictlanix/mbe-api/issues/76) rather than patched here. It has since landed; mbe-ui regenerated its client and suggestion rows now show photo/name/code/model/SKU (research §3).
...
| III. Contract-Driven API | ✅ Pass | Uses the already-generated `mergeProductsApiV1ProductsMergePost` + `ProductMergeRequest`; product search reuses the generated `listProductsApiV1ProductsGet`. No regeneration needed for the merge action itself; no hand-written DTOs. SKU-in-suggestion was tracked via an upstream issue ([mictlanix/mbe-api#76](https://github.com/mictlanix/mbe-api/issues/76), resolved) rather than hand-patched, then wired in after a real client regeneration. Errors mapped through the shared `mapDioException` → `AppError` and surfaced via the shared `error_banner`. |
...
*No outstanding deviations.* **SKU-in-suggestion-row** was tracked here as a deferred item (FR-003 shipping partial, pending [mictlanix/mbe-api#76](https://github.com/mictlanix/mbe-api/issues/76)) — that issue has since landed, mbe-ui regenerated its client, and FR-003 now ships in full (photo/name/code/model/SKU). The route-gated-on-Create choice (§IV design note) is a separate, non-violating tightening and remains recorded only in the Constitution Check and `contracts/routes.md`.

## Phase 0 — Research

See [research.md](./research.md). All Technical Context items are resolved; no `NEEDS CLARIFICATION` remain. Key decisions: reuse `ProductRepository.list` (with `deactivated: null`) as the picker search source; extend `CatalogEntityPicker` with optional photo/subtitle option rendering rather than forking a new widget; gate the merge route on Create; and ship suggestion rows with name/code/model/SKU — SKU was tracked via [mictlanix/mbe-api#76](https://github.com/mictlanix/mbe-api/issues/76) (resolved) rather than hand-patched, then wired in after a real client regeneration.

...
- Q: Should picker suggestion rows display the product's SKU, given it's one of the searchable fields? → A: **Yes, display it.** The products-list data the pickers reuse (`ProductListItem`) initially didn't carry a `sku` field (only `productId`, `code`, `name`, `model`, `brand`, `photo`). Rather than dropping the requirement, mbe-ui filed [mictlanix/mbe-api#76](https://github.com/mictlanix/mbe-api/issues/76) requesting `sku` be added to that projection — **resolved**: the field shipped upstream, mbe-ui regenerated its client, and FR-003's suggestion rows now display code, model, and SKU together.
...
- **SKU-in-suggestion-row dependency (resolved)**: FR-003's SKU display depended on mbe-api adding `sku` to the products-list (`ProductListItem`) projection — filed as [mictlanix/mbe-api#76](https://github.com/mictlanix/mbe-api/issues/76). Per this project's repo boundary (mbe-ui doesn't modify sibling repos directly), this was tracked as an external dependency rather than patched in mbe-ui. It has since landed upstream; mbe-ui regenerated its client and wired `sku` into the suggestion-row subtitle alongside code and model.

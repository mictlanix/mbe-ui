...
- Q: Should picker suggestion rows display the product's SKU, given it's one of the searchable fields? → A: **Yes, display it — this requires an mbe-api change, tracked upstream.** The products-list data the pickers reuse (`ProductListItem`) doesn't yet carry a `sku` field (only `productId`, `code`, `name`, `model`, `brand`, `photo`). Rather than dropping the requirement, mbe-ui filed [mictlanix/mbe-api#76](https://github.com/mictlanix/mbe-api/issues/76) requesting `sku` be added to that projection. FR-003 keeps SKU as a required display field; the UI-side work to consume it (client regen + domain-entity mapping + picker subtitle) is gated on that upstream issue landing (see Assumptions).
...
- **FR-003**: Each picker suggestion MUST display the product's photo thumbnail together with identifying text (name, code, model, SKU) so the operator can visually confirm the record. **Dependency**: SKU display requires mbe-api to expose `sku` on the products-list projection ([mictlanix/mbe-api#76](https://github.com/mictlanix/mbe-api/issues/76), open as of this writing); see Assumptions.
...
- **Duplicate**: The redundant record selected to be folded into the canonical product. Its transactional references and labels are remapped to the canonical product; its price rows and label rows are removed; the record itself is permanently deleted. Must be a different product than the canonical.
...
- **Backend and client already exist**: mbe-api's merge operation (`POST /api/v1/products/merge`, gated by the merge privilege with create-level access, returning no content on success) is implemented, and the generated API client already exposes it and its request shape. No API contract change or client regeneration is required for the merge action itself.
- **SKU-in-suggestion-row dependency**: FR-003's SKU display depends on mbe-api adding `sku` to the products-list (`ProductListItem`) projection — filed as [mictlanix/mbe-api#76](https://github.com/mictlanix/mbe-api/issues/76). Per this project's repo boundary (mbe-ui doesn't modify sibling repos directly), this is an external dependency to track, not something mbe-ui implements. Until it lands and mbe-ui regenerates its client, the picker's suggestion rows can only show photo/name/code/model; SKU remains searchable regardless (mbe-api's list search already matches on it) — see US2/FR-003.

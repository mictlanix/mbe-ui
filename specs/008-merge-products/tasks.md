...
- [ ] T016 [US2] Wire both merge pickers to the new params in `merge_products_screen.dart`: `optionImageUrl: (p) => p.photo`, `optionSubtitle: (p) => code/model`; verify min-length and no-results UX. **SKU deferred**: [mictlanix/mbe-api#76](https://github.com/mictlanix/mbe-api/issues/76) (add `sku` to `ProductListItem`) is open — FR-003 wants SKU in the subtitle too, but the field doesn't exist in the generated client yet. Ship this task with code/model only; once #76 lands, regenerate the client (`tool/generate_api_client.sh`), add `sku` to the domain `ProductListItem`, and extend the subtitle in a follow-up task (not numbered here — blocked on an external repo). Depends on T009, T015.

...
- **Post-implementation change (2026-07-18)**: at the product owner's request, the `pricing` nav destination was moved from the `catalogs` group into a new `sales` group (l10n key `salesGroupTitle`), while `price-lists` and `exchange-rates` stay under `catalogs`. Pure navigation grouping — no route, gate, or screen change. Updated in research.md §7 and contracts/routes.md; covered by new `app_navigation_test.dart` cases asserting the two groups are independently visible per privilege.

...
- Q: Given the four list endpoints expose no free-text `search`, how should users browse/find records while the constitution (§VI) forbids search-less catalog screens? → A: File mbe-api issues requesting a server-side `search` parameter on all in-scope endpoints and build the UI as if it exists (search box present on every screen, wired to that expected parameter). **Update 2026-07-19: RESOLVED — mbe-api shipped the `search` param on all three in-scope endpoints (issues #82/#83/#84) and the generated client was regenerated to include it. The dependency is closed; the search box is fully functional against server-side `search`, no longer pending.**
...
- How does a user find a specific record in these catalogs? → Every screen ships a search box wired to the server-side `search` parameter now available on all three list endpoints (FR-002), plus a driver filter for Vehicle Operators. The app MUST NOT ship a search-less catalog screen (§VI).
...
- **FR-002**: Each catalog's list screen MUST ship a free-text search box using the shared filter/search pattern, and MUST NOT ship search-less (constitution §VI). The search box filters by the record's identifying text (expense name; vehicle license plate/name/nickname; operator driver name/license number), wired to the server-side `search` parameter now available on all three list endpoints (regenerated into the client, mbe-api #82/#83/#84), consistent with the spec-012 catalogs.
...
- **Search & Browse** *(dependency resolved 2026-07-19)*: originally these three list endpoints exposed no free-text `search` parameter, so mbe-api issues #82/#83/#84 were filed and the UI was built against the expected parameter. **mbe-api has since shipped `search` on all three endpoints and the generated client was regenerated to include it**, so the search box on every screen is now fully functional against server-side search — no longer a pending cross-repository dependency. Vehicle Operators additionally exposes a driver/`employee` facet filter.

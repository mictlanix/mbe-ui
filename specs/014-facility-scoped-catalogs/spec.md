...
- Q: A facility's `taxpayer` is a required foreign key to a taxpayer-issuer record, but that table has **no endpoint** — it cannot be listed, searched, or resolved. How should the form handle it? → A: **Free-text RFC entry.** Unlike the opaque numeric address key that blocked the original attempt, a taxpayer key *is* the RFC — a value the operator knows and can type. The form accepts it as a validated 13-character RFC field and lets the server reject an unregistered one. This is a **deliberate, documented UX compromise and an explicitly temporary one**, not an oversight: the user gets no list of valid issuers and no confirmation until save. **A read-only taxpayer-issuer endpoint (list + search) has already been requested upstream — mictlanix/mbe-api#100 — specifically so this field can be upgraded to an autocomplete over registered issuers.** The interim field is therefore designed for that upgrade: the value stored is the registration key in both shapes, so only the input control changes when the endpoint lands (FR-034a).
...
**Why this priority**: Facilities is the parent of all three other catalogs, and today there is no way to manage one from the app at all — a facility has to be created by other means before a warehouse can point at it. That makes it genuinely valuable. It is P2 rather than P1 because the other three catalogs can ship and be tested against facility data that already exists, and because this story carries by far the largest form, the inline address sub-form, and the taxpayer compromise — so it should not gate the simpler slices.

**Independent Test**: Can be fully tested by opening the Facilities catalog, creating a facility with a new inline address and a known taxpayer RFC, editing its receipt message, changing its status, and deleting one — independent of the other three catalogs, though they consume its output.
...
6. **Given** a user entering a taxpayer that is not a registered issuer, **When** they save and the server rejects it, **Then** the rejection is shown clearly against the taxpayer field and no other input is lost.
...
- **FR-022**: The warehouse picker on the point-of-sale form MUST offer only warehouses belonging to the currently selected facility, and MUST require the user to reselect a warehouse when the facility is changed to one the current warehouse does not belong to. *(Resolved 2026-07-20: the backend performs no such validation and will accept a cross-facility pairing, so this guard is UI-only — it is the sole protection against an inconsistent pairing created through this app, and MUST NOT be described or tested as enforcing a backend invariant. An existing record that already pairs across facilities MUST still load and display rather than being rejected by the form.)*
...
- **FR-034**: The taxpayer field MUST, **as an interim measure only**, accept a directly typed taxpayer registration key (RFC) of at most 13 characters and MUST surface a server rejection of an unregistered key against that field (FR-012). The screen MUST NOT claim to validate the key's existence locally — only its shape. This is a temporary shape forced by the absence of any taxpayer-issuer endpoint; a read-only listable, searchable endpoint **has been requested upstream (mictlanix/mbe-api#100)** precisely so this field can become an **autocomplete** over registered issuers.
- **FR-034a**: The taxpayer field MUST be built so that its interim typed form can be replaced by an autocomplete over taxpayer issuers **without reworking the rest of the facility form** — the field's stored value is the registration key either way, so only the input control changes. Nothing else in the form, its validation, or its save path may depend on the field being free text.
- **FR-035**: Where a facility's address is displayed, the app MUST resolve it to readable address text rather than showing a raw id; because the facility response carries only the address key, this resolution MUST be performed without issuing one request per row on the list screen.
...
- **Taxpayer issuer** *(referenced, not managed; not browsable **yet**)*: The registered taxpayer a facility invoices as, identified by its registration key (RFC). No endpoint exposes these today, so the app can neither list nor validate them — the key is typed directly and the server is the only authority on whether it exists. A read-only endpoint has been requested (mictlanix/mbe-api#100) so this becomes an autocomplete; the entity's role in the facility form does not change when it does, only how the key is entered.
...
- **SC-004**: Zero point-of-sale records can be created *through this app* whose warehouse belongs to a facility other than the record's own. This is a UI guarantee only — the backend accepts such a pairing, so a record created by another client may still exhibit one and MUST display without error (FR-022).
...
- **SC-009**: 100% of displayed facility addresses render as readable address text or an explicit fallback, never as a raw id, and a facilities list page issues no more than one additional request to resolve all of its addresses.
...
- **Facilities management is IN scope** *(decision reversed 2026-07-20, see Clarifications)*: the three gaps that blocked it are resolved upstream — addresses are now a full CRUD catalog with search and their own `addresses(11)` object (mictlanix/mbe-api#90), `logo` is optional (#91), and the Store `0` / Production Site `1` mapping is confirmed (#92) — so Facilities is promoted to this feature's fourth managed catalog (User Story 4). Two consequences are accepted deliberately rather than treated as defects: the taxpayer field is typed rather than picked because no endpoint exposes taxpayer issuers (FR-034) — an **interim** shape, with the endpoint already requested (mictlanix/mbe-api#100) so it can become an autocomplete and the field built for that swap (FR-034a) — and addresses are created inline from the facility form rather than getting a catalog of their own (FR-031).
...
- The taxpayer registration key's maximum length: 13 characters.
- List-endpoint query parameters, all available in the regenerated client as of 2026-07-20: `search`, `facility`, `status`, `skip`, `limit` on all three; plus `warehouse` on points of sale. `search` is the first positional-optional param on every `list…` method.
...
### Follow-ups — filed 2026-07-20, none blocking

| Issue | Subject | Why it doesn't block |
|---|---|---|
| [#100](https://github.com/mictlanix/mbe-api/issues/100) | `taxpayer_issuer` has no endpoint — a required FK that cannot be listed, searched, or resolved. **Requested specifically to make the taxpayer field an autocomplete** | FR-034 ships the field as a typed 13-character RFC *in the interim*. FR-034a requires it be built so only the input control changes when this lands |
| [#101](https://github.com/mictlanix/mbe-api/issues/101) | `FacilityResponse.address` is not expanded, unlike every other FK — including `location` on the same model | FR-035 resolves addresses in bulk client-side. That workaround is deleted when this lands |
| [#102](https://github.com/mictlanix/mbe-api/issues/102) | Point of sale accepts a warehouse belonging to a different facility — no cross-FK validation | FR-022's picker guard covers this app. #102 also asks whether cross-facility pairing is ever legitimate; if it is, FR-022 relaxes to a warning |

- **`AddressType` is a new unnamed enum** (`number0`–`number4`), the same shape as `FacilityType` and `EntityStatus`. Handled UI-side by FR-033, so no upstream issue was filed.

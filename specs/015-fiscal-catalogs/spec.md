...
- Q: The `paymentMethod` field on a Payment Method Option is an integer SAT payment-method code (`c_FormaPago`), and `commission` is a string/number amount. Are these free entry? → A: `paymentMethod` is chosen from a standard, fixed list of SAT payment forms (`c_FormaPago`) presented with human-readable labels — **not** a free-typed code. *(Planning note 2026-07-21: the SAT catalog service exposes no payment-forms endpoint, so this list is a fixed labeled lookup rather than a live picker; see plan research §5. The member set is confirmed — mbe's legacy PaymentMethod enum: 0 N/A default, 1 Efectivo, 2 Cheque, 3 Transferencia Electrónica, 4 T. de Crédito, 5 Monedero Electrónico, 6 Dinero Electrónico, 8 Vales de Despensa, 12 Dación, 27 A Satisfacción Del Acreedor, 28 T. de Débito, 29 Tarjeta de Servicio, 30 Aplicación de Anticipos, 99 Por Definir, 1001 FONACOT.)* `commission` is an optional decimal amount entered on the form. `numberOfPayments` defaults to 1 and `displayOnTicket` defaults to true.
- Q: Do all three list endpoints expose free-text search, which the design system requires on every catalog? → A: Only the Taxpayer Issuers list does. The Payment Method Options and Taxpayer Certificates lists currently expose only their facet filters (facility/status and taxpayer/status respectively) with no free-text `search`. Following the spec-013/014 precedent, each screen still ships a search box wired to an expected server-side `search` capability, and the two gaps are filed upstream as a tracked dependency (see Dependencies). This is not a design-system deviation and not a client-side filtering workaround.
...
- **FR-026a**: All three catalog list screens MUST present a free-text search box alongside their facet filters, per the shared filter pattern. The Taxpayer Issuers search operates server-side today; the Payment Method Options and Taxpayer Certificates search boxes are wired to an expected server-side search capability tracked as an upstream dependency (see Dependencies) and MUST NOT be implemented as client-side filtering of the current page.
...
- The SAT catalog picker path already used by the product and facility forms (fiscal regime, postal code) is reused unchanged for the issuer form. The SAT payment form (`c_FormaPago`) has no catalog endpoint, so the payment-method-option form presents it as a fixed labeled list rather than a live picker (plan research §5).
...
- **Upstream (tracked, non-blocking)**: The Payment Method Options and Taxpayer Certificates list endpoints do not yet accept a free-text `search` parameter (only their facet filters). Following the spec-013/014 precedent, requests are filed upstream to add `search` to both list endpoints; each screen ships its search box wired to that expected capability, activating automatically once it lands. This is a tracked dependency, not a design-system deviation. The Taxpayer Issuers list already accepts `search`.

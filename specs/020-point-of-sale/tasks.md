...
- [X] T066 [US2] **Two spec assumptions corrected against the live API** — `DeliveryOrderCreate` cannot carry `shipTo`/`contact`/`date`/`comment` (create-then-PUT, with cancel-on-failure rollback; [mbe-api#146](https://github.com/mictlanix/mbe-api/issues/146)), and a delivery order exposes no `sales_order` link at all, so `listForSale` reconstructs it from line ids ([mbe-api#147](https://github.com/mictlanix/mbe-api/issues/147)). Implement `DeliveryOrderRepositoryImpl` in `lib/features/sales/data/delivery_order_repository_impl.dart` wrapping the generated `DeliveryOrdersApi`, passing `contact` as a real id — **no comment-encoding stopgap** (resolved, research §10) — depends on T065

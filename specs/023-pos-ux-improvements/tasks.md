...
- [ ] T064 Run the `quickstart.md` manual validation pass (all 22 checks) at 1440 px, ~1024 px, ~800 px and 390 px (depends on T063). **Not run** — this is an interactive pass against a real signed-in session (register assignment, an open cash session, seeded draft/confirmed/paid sales) in an actual browser, judged by eye across 4 window widths; nothing in this environment can drive that (no browser/session available to this agent). Everything automatable stands in for it as far as it can: `pos_capture_golden_test.dart` (T059) pixel-captures checks 20–22 at exactly the widths this checklist asks for, and the widget/unit/integration suites cover the logic behind checks 1–19. Left for a human to run before calling the feature done.

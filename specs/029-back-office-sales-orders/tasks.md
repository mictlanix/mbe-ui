...
- [ ] T029 [US1] Wire confirm into `order_screen.dart`: disabled until `lineCount > 0` (FR-023), rendering a refusal's every named line in the banner without leaving the screen (FR-024), and flipping the screen to its read-only face with the folio shown on success (FR-025) (depends on T026)
...
- [ ] T043 [US2] Add the cancel action to `lib/features/sales/presentation/orders/order_screen.dart` via `RecordFormActions` — on the order's own screen only, behind an explicit confirmation dialog, with a server refusal (paid order, live payments, already cancelled) rendered in the same banner (FR-026)
...
- [ ] T057 [US4] Render the payment facts an order carries — outstanding balance and paid state — on the order screen without leaving it, reading them off the `Sale` the server already returns (FR-026; no payments endpoint is called, OS-2)
- [ ] T058 [US4] Gate every mutating affordance in `order_screen.dart`, `order_header_panel.dart` and `sales_orders_list_screen.dart` on `can(salesOrders, create|update)`, hidden not disabled (constitution §IV, contracts/routes.md §5)
...
- [ ] T061 [US5] Decode the `salesperson` and `facility` facets in `SalesOrdersFilter.fromQuery` **only when `isAdministrator`** — a non-administrator's filter carries neither, whatever the URL says; this is where the hand-edited-address edge case is closed, not in the drawer (data-model.md §4.1, FR-006)
...
- [ ] T073 Walk [quickstart.md](quickstart.md)'s manual validation for all five stories with all four accounts, at both the expanded and compact width tiers

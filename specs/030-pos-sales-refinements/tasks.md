...
- [ ] T001 [P] Add `posEditDestinationSheetTitle` ("Editar destino") and
...
- [ ] T002 [P] Add the same two keys, English text ("Edit destination" /
...
- [ ] T004 [P] [US1] Unit tests for `QuantityStepperController` in
...
- [ ] T008 [US1] In the same file, add `QuantityStepper extends
...
- [ ] T010 [P] [US1] In `lib/features/sales/presentation/capture/sale_line_row.dart`:
...
- [ ] T011 [P] [US1] In `lib/features/sales/presentation/capture/sale_line_card.dart`:
...
- [ ] T013 [US1] Run `flutter test test/golden/pos_capture_golden_test.dart`
...
- [ ] T017 [US2] In `quantity_stepper.dart`'s `QuantityStepperController`: add
...
- [ ] T018 [US2] In `QuantityStepper`'s `State`: wire a `FocusNode` around the
...
- [ ] T019 [US2] In the same `State`: on a `resetTick` change, run a 250 ms
...
- [ ] T022 [P] [US3] Add a `group('updateHeader —` block to
...
- [ ] T026 [US3] Add `Future<Destination> updateDestination({required int
...
- [ ] T032 [P] [US4] Extract the read-only line-row shape from
...
- [ ] T034 [US4] In the same file: compute `storeShare(line)` per
...
- [ ] T035 Run `flutter analyze` clean across the changed files.
- [ ] T036 Run the full suite — `flutter test` — and confirm
  ...
  R12, plan's Constitution Check).
- [ ] T037 Work through
  [quickstart.md](./quickstart.md) §3 and §5 by hand against
  `flutter run -d chrome --dart-define-from-file=.env` at 1440/1024/380 px,
  including the largest text-size level and the OS reduce-motion setting
  (spec FR-032, SC-008).
- [ ] T038 Work through [quickstart.md](./quickstart.md) §4 and §6 against a
  live mbe-api: edit a real destination's date and address, confirm a 409 on
  a confirmed order renders the server's own message, and confirm a burst of
  taps produces exactly one `PUT` in the server log.

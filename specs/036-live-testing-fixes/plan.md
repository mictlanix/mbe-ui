...
| Product-lookup's `warehouse:` filter behavior on `stock` is unverified against a live backend | If it filters to one warehouse, every non-default warehouse shows permanently "unknown" and US6 delivers little value | Verify against a live mbe-api instance before implementing the warehouse-picker flag (research R11); fall back to a per-warehouse stock fetch if confirmed |

...
### User Story 9 - One configurable pace governs every debounced field (Priority: P3)
...
or adjusting before acting. Nothing lets a deployment tune that pace in one place.

**Why this priority**: A tuning knob with no reported malfunction today — worth having, but
the lowest-impact item here.

**Independent Test**: Change the debounce-duration setting and confirm every debounced field's
behavior shifts together, with no field left on its own separately-hardcoded delay.

**Acceptance Scenarios**:

1. **Given** the debounce-duration setting at its default, **When** a debounced field is used,
   **Then** it waits the same, current delay it does today (no behavior change for a deployment
   that leaves the setting unset).
2. **Given** the setting changed to a different value, **When** the app is restarted, **Then**
   every debounced field across the app waits the new delay — none is left on its own
   hardcoded value.
...
- A deployment sets the debounce-duration setting to zero or a very large value — the setting
...
- **SC-008**: Changing the deployment's debounce-duration setting changes 100% of debounced

...
- [X] T002 Add `bool isGenericCustomer(int customerId)` to `lib/core/config/app_settings.dart`
...
- [X] T003 [US1] In `lib/features/sales/presentation/orders/order_screen.dart`, add
...
- [X] T004 [US1] In `lib/features/sales/presentation/capture/customer_bar.dart`, add
...
- [X] T006 [P] [US1] Update `test/widget/features/sales/order_screen_test.dart`: a brand-new order
...
- [X] T007 [P] [US1] Update `test/widget/features/sales/customer_bar_test.dart`: with
...
- [X] T008 [US1] Fix the existing `order_screen_test.dart` cases that assumed the search field is
      always present on mount (research.md R5 flags this as a known break from T003).
- [X] T071 [P] [US1] *(added post-`/speckit-analyze`, closes FR-004 coverage gap)* Add a
...
- [X] T009 [US2] In `lib/features/sales/presentation/pos_step_controller.dart`, add
...
- [X] T010 [US2] In `lib/features/sales/presentation/sale_editing.dart` /
...
- [X] T011 [US2] In `lib/features/sales/presentation/payment/payment_controller.dart`, call
      ...
      C1).
- [X] T012 [US2] In `lib/features/sales/presentation/delivery/delivery_controller.dart`, at the
...
- [X] T013 [US2] At the credit-terms leave-Cobro path (wherever `canLeavePayment`/
      `advanceFromCobro` is invoked, e.g. `payment_step.dart`), call `confirm()` before leaving
      Cobro when no cash payment was posted.
- [X] T014 [US2] In `lib/features/sales/presentation/pos_workspace_screen.dart`, make the Venta
...
- [X] T017 [P] [US2] Add cases to `test/unit/features/sales/pos_step_controller_test.dart` for
      `canReturnToCapture`/`returnToVenta`.
- [X] T018 [P] [US2] Add a `PaymentController.submit` test asserting `confirm()` is called exactly
      once, before `createPayment`, and not again on a second tender.
- [X] T019 [P] [US2] Add a delivery-step test asserting `confirm()` precedes the first
      delivery-order create.
- [X] T020 [P] [US2] Add a widget test: a draft sale on Cobro exposes the Venta pill/affordance; a
      sale with ≥1 non-cancelled payment does not.
- [X] T021 [US2] Update `test/integration/pos_counter_sale_flow_test.dart`,
      `pos_delivery_split_flow_test.dart` and `pos_resume_flow_test.dart` for the new call
      ordering (they currently assert `confirm()` at Venta→Cobro). **Finding:** no change needed —
      all three call `salesOrderRepository.confirm()` directly to assert the mbe-api contract
      itself (folio assignment, editability freeze), never through `PosStepController`/
      `confirmBeforePayableAction`, so the presentation-layer call-ordering change does not touch
      them.
...
- [X] T022 [US3] In `lib/features/pricing/presentation/pricing_grid_controller.dart`, add
...
- [X] T023 [US3] In `lib/features/pricing/presentation/price_cell.dart`, wire the `TextField`'s
...
- [X] T024 [P] [US3] Add to `test/widget/features/pricing/price_cell_test.dart`: type into cell A,
      click directly into cell B → exactly one write for A's value, B ends active.
- [X] T025 [P] [US3] Add the same case via keyboard (Enter/Tab/arrow) asserting exactly one write
      (no double-commit from T022's guard).
- [X] T026 [P] [US3] Confirm the existing Escape-discards-nothing test still passes unchanged.
- [X] T027 [P] [US3] Add an unmount-while-active case (page/filter change) that still commits.
- [X] T028 [P] [US3] Add an invalid-value-then-tap-away case: the rejected badge/reason renders
      (FR-010).
...
- [X] T029 [US4] Check whether [mbe-api#198](https://github.com/mictlanix/mbe-api/issues/198) and
...
- [X] T072 [US4] *(added post-`/speckit-analyze`, closes a constitution Development-Workflow gap)*
...
- [X] T030 [US4] **Depends on T033 landing first (or in the same commit)** — dropping the two
      ...
      `""`.
- [X] T031 [US4] In `lib/features/catalog/presentation/customer_form.dart`: move the `code` field
      to immediately after `credit_days`; remove the two shipping `SwitchListTile`s.
- [X] T032 [P] [US4] In `lib/features/catalog/domain/entities/customer.dart` (and
      `customer_list_item.dart` if applicable), drop `shipping`/`shippingRequiredDocument`.
- [X] T033 [US4] **Not parallel with T030 — do this first, or in the same commit**: T030 stops
...
- [X] T034 [P] [US4] In `lib/features/sales/presentation/customer_inline_create.dart` (POS
      inline-create mini-form), remove the two shipping switches.
- [X] T035 [P] [US4] Remove the orphaned l10n keys (`shippingLabel`,
      ...
      reason.
- [X] T036 [US4] In `lib/features/sales/presentation/capture/fulfillment_mode_selector.dart`,
...
- [X] T037 [US4] In `lib/features/sales/presentation/capture/customer_bar.dart`'s
...
- [X] T038 [P] [US4] Update `test/unit/features/catalog/customer_form_controller_test.dart`: code
      not required; create/update stub assertions drop the shipping args.
- [X] T039 [P] [US4] Update `test/widget/features/catalog/customer_form_test.dart`: no shipping
      switches render; `code_field`'s position is after `credit_days_field`; saving with an empty
      code succeeds.
- [X] T040 [P] [US4] Update `test/widget/features/sales/customer_inline_create_test.dart`: remove
      the 8 shipping-arg mocktail stubs; add a no-code save case.
- [X] T041 [P] [US4] Rewrite `test/widget/features/sales/fulfillment_mode_selector_test.dart`'s
...
- [X] T042 [P] [US4] Add a `customer_bar_test.dart` case: switching to the generic customer while
...
- [X] T043 [US5] In `customer_bar.dart`'s customer-picker `onSelected` (same `_updateHeader` call
...
- [X] T044 [US5] In `lib/features/sales/presentation/orders/order_header_panel.dart`, pass
      `initialDisplayText` from the resolved customer's salesperson name so an autofilled value
      renders, not a blank field.

### Tests for User Story 5

- [X] T045 [P] [US5] Add cases (order_screen_test.dart / customer_bar_test.dart): selecting a
...
- [X] T046 [US6] **Spike, do first**: verify against a live mbe-api whether the product-lookup
...
- [X] T047 [US6] In `lib/features/sales/presentation/capture/sale_line_editing.dart`, extend
...
- [X] T048 [US6] In the same file's `warehousePicker()`, render the flag per `DropdownMenuItem`
...
- [X] T050 [P] [US6] Add to `test/widget/features/sales/sale_line_row_test.dart`: the four stock
...
- [X] T052 [US7] In `lib/features/sales/presentation/delivery/delivery_controller.dart`'s
...
- [X] T053 [P] [US7] Add a `delivery_controller` unit test: first destination's `create` carries
      every line's `claimable`; second destination sends `[]`; a refused create adds nothing.
- [X] T054 [P] [US7] Add to `test/widget/features/sales/destination_assignment_test.dart`: after
...
- [X] T055 [US8] *(sequence after Phase 5 — same file/functions as T022/T023)* In `price_cell.dart`,
      ...
      C3).
- [X] T056 [P] [US8] Apply the same fix to `lib/features/pricing/presentation/pricing_screen.dart`'s
      single-product edit dialog.
- [X] T057 [P] [US8] In `pricing_grid_controller.dart`, parameterize the adjust-percent rounding on
      `currencyDecimalDigits` instead of a hardcoded `2`.
- [X] T058 [P] [US8] Audit `lib/features/sales/presentation/payment/payment_capture_pane.dart` for
      the same raw-seed/raw-commit bypass; fix if found.

### Tests for User Story 8

- [X] T059 [P] [US8] Add/update widget and unit tests: a `20.0000`-stored price renders as `20.00`
...
- [X] T060 [US9] In `lib/core/config/app_settings.dart`, add `inputDebounce`
...
- [X] T061 [US9] Add `inputDebounceProvider` and `quantityCommitDebounceProvider` (derived from
      `appSettingsProvider`) beside `formattersProvider`.
- [X] T062 [P] [US9] Convert `lib/core/widgets/catalog_entity_picker.dart` to
      `ConsumerStatefulWidget` and read `inputDebounceProvider` for its debounce timer.
- [X] T063 [P] [US9] In `lib/features/sales/presentation/capture/product_search_field.dart`, read
      `inputDebounceProvider` instead of the hardcoded 300ms.
- [X] T064 [P] [US9] In `lib/features/sales/presentation/widgets/quantity_stepper.dart`, add a
...
- [X] T065 [P] [US9] Document both new env vars, with defaults and one-line descriptions, in
      `.env.template`.

### Tests for User Story 9

- [X] T066 [P] [US9] Add an `AppSettings` parse-table unit test for both new fields: valid, empty,
      non-numeric, negative → documented default.
- [X] T067 [P] [US9] Update `catalog_entity_picker_test.dart`, `product_search_field_test.dart`,

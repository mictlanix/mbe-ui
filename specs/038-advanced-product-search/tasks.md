...
- [ ] T001 Confirm `flutter analyze && flutter test` pass on the current branch before any change, as
...
- [ ] T002 [P] Add `productSearchMultiSelect` (`bool`, default `true`) to `AppSettings` in
...
- [ ] T003 [P] Add `productSearchMultiSelectProvider` to `lib/core/config/app_settings_provider.dart`,
      mirroring `inputDebounceProvider`'s one-line pattern (depends on T002).
- [ ] T004 [P] Document `PRODUCT_SEARCH_MULTI_SELECT` in `.env.template`'s app-settings block, near
      `POS_DEFAULT_CUSTOMER_ID`, with the two-line comment from
      contracts/app-settings-additions.md C1.
- [ ] T005 [P] Add `PRODUCT_SEARCH_MULTI_SELECT=true` to `.env.settings` so the VS Code run
      configuration exercises it (quickstart.md "Run").
- [ ] T006 Add two tests to `test/unit/core/config/app_settings_test.dart`: (a) add
...
- [ ] T007 [P] Add the 8 new keys from contracts/advanced-search-screen.md §5
      ...
      `@key` metadata object.
- [ ] T008 Add the same 8 keys, in English, to `lib/l10n/app_en.arb` (depends on T007 — this
      project's convention is Spanish first, then English).
- [ ] T009 Run `flutter gen-l10n` to regenerate `AppLocalizations` (depends on T007, T008) — a
      missed regeneration produces a stale getter, not a compile error.
- [ ] T010 [P] Create `lib/features/sales/presentation/capture/advanced_search_state.dart` with
...
- [ ] T011 [P] In `lib/features/sales/presentation/capture/product_search_field.dart`, change
...
- [ ] T012 [US1] Add a new top-level `GoRoute(path: '/sales/product-search', builder: (context,
...
- [ ] T013 [US1] Create `lib/features/sales/presentation/capture/advanced_search_screen.dart`:
...
- [ ] T014 [US1] In the same file, render the table via `CatalogListStateView<ProductListItem>` fed
      by `productsListControllerProvider(filter)`, using `DataTableView<ProductListItem>` with: a
      leading `DataTableColumn` (`fixedWidth: 56`, `cellBuilder` returning
      `IgnorePointer(child: Checkbox(value: selected, onChanged: null))`), then photo (120), code
      (200), name (L), brand (S), unit (M) — **no** status column, **no** `rowActionsBuilder`.
      `onRowTap` toggles the tapped product in `advancedSearchSelectionProvider` by `productId`
      (append if absent, remove if present) — the checkbox cell has no handler of its own, so there
      is exactly one way to toggle a row (contracts/advanced-search-screen.md §2, §3; research.md
      R4).
- [ ] T015 [US1] In the same file, add a bottom action bar: `advanced_search_selection_count` (the
      ...
      `advancedSearchResultProvider` to the current selection (preserving tick order) and pops.
- [ ] T016 [US1] In `lib/features/sales/presentation/capture/product_search_field.dart`, add the
...
- [ ] T017 [US1] In the same file, add `ref.listen(advancedSearchResultProvider, (prev, next) {
      ...
      propagating it. After the loop, render `advanced_search_skipped` (one `code — name` per
      skipped product; nothing when the list is empty) and clear `_advancedAdding`
      (contracts/advanced-search-screen.md §4; data-model.md §2, §7; research.md R3).

### Tests for User Story 1

- [ ] T018 [P] [US1] Widget tests in `test/widget/features/sales/advanced_search_screen_test.dart`
      ...
      Cancel pops leaving the result provider `null`.
- [ ] T019 [P] [US1] Widget tests in `test/widget/features/sales/product_search_field_test.dart`: the
...
- [ ] T020 [US1] Widget test proving research.md R1/R2's core property — pump the field inside
      ...
      `context.go`, is what ran (depends on T013).

**Checkpoint**: User Story 1 is fully functional and independently testable — the MVP slice.
...
- [ ] T021 [US2] In `advanced_search_screen.dart`, add the badged filters button to the
      ...
      `_replaceWith` with `pageIndex: 0`.
- [ ] T022 [US2] In the same file, compute the filters badge as
...
- [ ] T023 [US2] Wire `productLabelFacetsProvider(filter)` with the **same** forced `filter`
      (`status: active, salable: true`) the table uses, so label chip counts match exactly what the
      table can reveal (research.md R5).
- [ ] T024 [US2] Wire the filter sheet's `onClearAll` to `_replaceWith` the bare
      `/sales/product-search` (search cleared, no facets), matching
      `showCatalogFilterSheet`'s `onClearAll` contract.

### Tests for User Story 2

- [ ] T025 [P] [US2] Widget tests in `advanced_search_screen_test.dart`: the filter panel shows
      Stockable, Purchasable, Supplier and Label and neither Status nor Salable; a supplier + one
      label selected shows a badge of 2 (not 4); applying them narrows the table.
- [ ] T026 [P] [US2] Widget test: a `ListQuery` hand-constructed with `status=all` and
      `salable=false` facets still yields a table restricted to active + salable products — the
      forced facet cannot be overridden by the URL (FR-008).
...
- [ ] T027 [US3] In `advanced_search_screen.dart`, verify/adjust the row-tap toggle from T014 reads
...
- [ ] T028 [US3] Add the running count and a `advanced_search_clear_selection` action to the bottom
...
- [ ] T029 [US3] Confirm (adjust `product_search_field.dart`'s T017 loop if needed, no behavioural
...
- [ ] T030 [P] [US3] Widget test: tick two products on page 1, page to page 2 and tick a third,
...
- [ ] T031 [P] [US3] Widget test: one selected product's lookup returns no matching row (simulate via
...
- [ ] T032 [P] [US3] Widget test: "Clear selection" empties the count and every tick without
      navigating away.
...
- [ ] T033 [US4] In `advanced_search_screen.dart`'s row-tap handler, branch on
...
- [ ] T034 [P] [US4] Widget test with `productSearchMultiSelectProvider` overridden to `false`:
      ticking a second row releases the first tick; the selection never exceeds length 1 and the Add
      button reads "Add (1)".
- [ ] T035 [P] [US4] Widget test with the provider at its default (`true`, no override): unaffected —
      still multi-select (regression guard against T033).
- [ ] T036 [US4] Manual verification per quickstart.md V5: run with
...
- [ ] T037 [P] [US5] Widget test in `product_search_field_test.dart`: with `accessControlProvider`
      overridden so `can(SystemObject.products, AccessRight.read)` is `false`, the Advanced search
      button is absent from the field.
- [ ] T038 [P] [US5] Widget test: with the field's own `enabled: false`, the Advanced search button
      is also disabled (FR-003) — confirms T016's `widget.enabled` wiring.
- [ ] T039 [P] [US5] Test (alongside this repo's existing router-gate tests, wherever `_routeGate` is
...
- [ ] T040 [P] Compact-layout widget test in `advanced_search_screen_test.dart` using
...
- [ ] T041 [P] Confirm the existing scan/type-ahead widget tests in `product_search_field_test.dart`
...
- [ ] T042 Check `test/widget/features/sales/pos_compact_layout_test.dart`'s existing
...
- [ ] T043 Run `flutter analyze && flutter test` for the full suite; fix any fallout from the
...
- Within US1: T018, T019 (test files) can run in parallel with each other once T012–T017 land; T020
  depends specifically on T013.
...
from a paged (unfiltered-by-user) table of active/salable products, confirm, and get a correctly
priced line — with the sale intact if they cancel instead. This alone is shippable; the deployment
setting simply defaults to multi-select, which behaves identically to single-select when only one
row is ever ticked.

### Recommended Order

1. **Setup + Foundational** (T001–T011) — nothing compiles against the new contract without this.
2. **User Story 1** (T012–T020) — MVP. Validate independently before continuing.
3. **User Story 2** (T021–T026) — filters, additive to the same screen file.
4. **User Story 3** (T027–T032) — multi-select UX, additive and independent of US2.
5. **User Story 4** (T033–T036) — the configuration gate, small and low-risk once US1's tick handler
   exists.
6. **User Story 5** (T037–T039) — verification of gating already in place; catches any gap before
   calling the feature done.
7. **Polish** (T040–T043) — compact layout, regression guards, final full-suite run.
...
- Every task lists its exact file path except T001 (a whole-suite baseline check) and T036/T043

...
- [ ] T016 [P] Update the register's `CaptureStep` call site
...
- [ ] T017 [P] In `lib/features/sales/presentation/capture/product_stock_cache.dart`,
...
- [ ] T018 Extract the generic ~15 lines of
...
- [ ] T019 [P] Create `lib/features/sales/presentation/sales_quote_write_scope.dart`:
...
- [ ] T020 [P] Create `lib/features/sales/domain/entities/sales_quote_summary.dart`:
...
- [ ] T021 Create `lib/features/sales/domain/repositories/sales_quote_repository.dart`:
      ...
      — contracts/sales-quote-repository.md §1 (depends on T006, T007, T020)
- [ ] T022 [P] Promote the five `anyOf: [string, num]` wire-value setters
      ...
      §3
- [ ] T023 Create `lib/features/sales/data/sales_quote_repository_impl.dart`:
      ...
      on T021, T022)
- [ ] T024 In `sales_quote_repository_impl.dart`, add `_toQuoteError`: a 422
      ...
      §2 (depends on T023). **This is the highest-risk omission in the
...
- [ ] T025 Create `lib/features/sales/presentation/quote_editing.dart`:
      ...
      comment each ("quotes carry no …") — research.md R3 (depends on T018,
      T023)
- [ ] T026 Create `lib/features/sales/presentation/quotes/quote_editor_controller.dart`:
      ...
      contracts/sales-quote-repository.md (depends on T025)

### Screens and routing scaffolding

- [ ] T027 [P] In `lib/core/navigation/nav_destinations.dart`: add
      ...
      R6
- [ ] T028 [P] Add l10n key `salesQuotesMenuTitle` ("Sales Quotes"/
      "Cotizaciones") to `lib/l10n/app_en.arb` and `app_es.arb`, in parity —
      used for both the nav label and the app-bar title
- [ ] T029 Create `lib/features/sales/presentation/quotes/quote_screen.dart`:
      ...
      excludeGenericCustomer: true, showWarehouse: false)` — `onContinue` is
      wired in US1; mirrors `order_workspace_screen.dart`'s shell shape. This
      ...
      only configures (depends on T026, T012)
- [ ] T030 [P] Create `lib/features/sales/presentation/quotes/sales_quotes_list_controller.dart`:
      ...
      §4, research.md R5 (depends on T020, T021)
- [ ] T031 [P] Create `lib/features/sales/presentation/quotes/sales_quotes_list_screen.dart`:
      a skeleton screen (no columns/rows yet — added in US3) so the router has
      a builder target (depends on T030)
- [ ] T032 In `lib/app/router/app_router.dart`: append a new last
      ...
      matches this prefix and is gated the same way) (depends on T027, T029,
      T031)

### Test infrastructure

- [ ] T033 [P] Extend `test/widget/features/sales/pos_test_harness.dart`:
...
- [ ] T034 Generalize `test/widget/features/sales/sale_editor_isolation_test.dart`
...
**unchanged assertions**; the three-host isolation test (T034) passes. User
...
- [ ] T035 [P] [US1] Unit test in
      ...
      T024 (plain-string 422 → `creditHold`, map-with-message 409, plain
      string 409) — research.md R4
- [ ] T036 [P] [US1] Widget test in
...
- [ ] T039 [US1] In `quote_screen.dart`, wire `onContinue` to call
      ...
      once confirmed rather than being greyed out; render the header facts
      (customer, payment terms, expiry, currency, comment) — FR-003, FR-018,
      FR-020, FR-021, FR-023
- [ ] T040 [P] [US1] Add l10n keys: `salesQuoteConfirmAction`,
      `salesQuoteStatusDraft`, `salesQuoteStatusCompleted`,
      `salesQuoteStatusCancelled` — research.md R6
...
- [ ] T041 [P] [US2] Widget test in `quote_screen_test.dart`'s convert group:
...
- [ ] T042 [P] [US2] Extend `sales_quotes_flow_test.dart` (T038): confirm →
...
- [ ] T043 [US2] In `quote_screen.dart`, add the Convert action: visible only
      when `status == completed && !hasExpired` and `access.can(salesOrders,
      create)` (FR-025, FR-004); on success, `context.go('/sales/orders/
      $newOrderId')` (T026's `convert()`) — the order workspace resumes new
      drafts on its goods step by construction (FR-027); on failure, render
      the server's message via `ErrorBanner` (FR-028); when `hasExpired`,
      additionally render a Duplicate call-to-action beside the refusal
      (FR-029)
- [ ] T044 [P] [US2] Add l10n key `salesQuoteConvertAction`
...
- [ ] T048 [P] [US3] Widget test: reopening a draft resumes it editable with
...
- [ ] T049 [P] [US3] Router test group in
...
- [ ] T050 [US3] Flesh out `sales_quotes_list_screen.dart`: `CatalogFilterBar`
      with a search box and status/customer facets (`CatalogEntityPicker` for
      customer); `DataTableView<SalesQuoteSummary>` with the six columns
...
- [ ] T051 [P] [US3] Create `SalesQuoteStatusChip` (built on
      `core/widgets/status_chip.dart`) plus a separate expired-marker widget
      — FR-024, FR-037
- [ ] T052 [US3] In `quote_screen.dart`, add the Cancel action, visible only
      ...
      (research.md R5)
- [ ] T053 [P] [US3] Add l10n keys: `salesQuotesColumnReference`,
...
- [ ] T054 [P] [US4] Widget test: from `quote_screen.dart`'s customer band,
...
- [ ] T055 [US4] **Needs no new code.** `CustomerBar`'s existing inline-create
      ...
      `quote_screen.dart` (T029) already sets. This task is verification
      only: confirm T054 passes with no change to `customer_bar.dart` or
      `quote_screen.dart`
...
- [ ] T056 [P] [US5] Widget test: duplicating a quote in any state (draft,
...
- [ ] T057 [P] [US5] Widget test: from an expired quote's failed-convert
      state (T041), the offered Duplicate action navigates to the new draft

### Implementation for User Story 5

- [ ] T058 [US5] In `quote_screen.dart`, add the Duplicate action, gated on
      `access.can(SystemObject.salesQuotes, AccessRight.create)` (FR-003) and
      offered in any state (FR-033); on success, `context.go('/sales/quotes/
      $newQuoteId')` (T026's `duplicate()`)
- [ ] T059 [P] [US5] Add l10n key `salesQuoteDuplicateAction`
...
- [ ] T060 [P] Re-run the three-host isolation test (T034) after all five
      stories have landed; confirm it is still green
- [ ] T061 [P] Confirm `git diff --stat main --
      ...
      plan documents (T008, T009, T011, T016) — no incidental change to
      either host
- [ ] T062 [P] `flutter analyze` clean; full suite green, including T002's
      baseline re-run with unchanged assertions
- [ ] T063 Run quickstart.md's full validation: the automated commands, then
      ...
      target deployment
- [ ] T064 [P] Diff `lib/l10n/app_en.arb` and `app_es.arb` against their
      pre-feature state; confirm no key outside this feature's own changed
- [ ] T065 Manually confirm each of SC-001 through SC-008 against the
...
- **US2 (P1)**: depends on Foundational's `QuoteEditorController.convert()`
  (T026) and `quote_screen.dart` (T029) existing — not on the rest of US1
- **US3 (P2)**: depends on Foundational's list controller/screen shells
  (T030, T031) and `QuoteEditorController.cancel()` (T026) — not on US1/US2
- **US4 (P2)**: depends on Foundational's `quote_screen.dart` (T029) only;
...
- **US5 (P3)**: depends on Foundational's `QuoteEditorController.duplicate()`
  (T026) and `quote_screen.dart` (T029); its second test (T057) also touches
  US2's expired-convert refusal (T041) but does not depend on its
  implementation
...
- The capture-step change (T012–T017) before the quote screen that relies on
  `showWarehouse: false` (T029)
- The repository (T020–T024) before the controller (T025–T026)
- The controller and screen shells (T025–T029) before any story's screen work

### Parallel Opportunities

- Within Phase 2: T003, T004, T008–T011, T013, T014, T016, T017, T019, T020,
  T022, T027, T028, T030, T033 are marked `[P]` — different files, no
  cross-dependency
- Within each story's test block, all `[P]`-marked tests can be written
  together
- US3's list work (T045–T053) and US4's inline-create verification
  (T054–T055) can proceed concurrently with US2 (T041–T044) once Phase 2
...
Task: "Add NavBranch.salesQuotes + destination entry (T027)"
Task: "Promote the 5 anyOf wire-value setters, add 2 for priceAdjustment (T022)"
```
...
4. **STOP and VALIDATE**: quickstart.md's US1 section (steps 1–8), plus
   T034's isolation assertion — confirm the register and the order workspace
...
- T024 and T034 are this feature's two most important checks — not because
  they are hard to write, but because each is the only place a specific
  silent failure would be caught: T024 catches a destroyed error message
  before it reaches a live user with no point of sale configured; T034

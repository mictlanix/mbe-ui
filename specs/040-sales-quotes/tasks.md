...
- [X] T001 Confirm `040-sales-quotes` is checked out and run `flutter pub get`
      at the repository root
- [X] T002 [P] Run `flutter test test/unit/features/sales test/widget/features/sales test/integration` and `flutter analyze`, and record the baseline pass
...
- [X] T003 [P] In `lib/features/sales/domain/entities/sale.dart`: make
...
- [X] T004 [P] In `lib/features/sales/domain/entities/sale_line.dart`: make
...
- [X] T005 Run `dart run build_runner build --delete-conflicting-outputs` to
      regenerate `sale.freezed.dart` and `sale_line.freezed.dart` (depends on
      T003, T004)
- [X] T006 [P] Add `Sale.fromQuoteResponse(api.SalesQuoteResponse r)` factory
...
- [X] T007 [P] Add `SaleLine.fromQuoteLineResponse(api.SalesQuoteLineResponse
      r)` factory to `sale_line.dart`: maps `priceAdjustment`, leaves
      `cost`/`unit`/`photo`/`warehouse` null (depends on T005)
- [X] T008 [P] Fix the 3 now-nullable `pointSale` reads in
      `lib/features/sales/presentation/pos_workspace_screen.dart:231,253,277`
      (depends on T005)
- [X] T009 [P] Fix the 3 now-nullable `promiseDate`/`priority` reads in
      `lib/features/sales/presentation/orders/order_header_panel.dart:162,410,222`
      (depends on T005)
- [X] T010 [P] Replace the 10 `sale.balance` reads with `sale.balanceOrZero`
      ...
      (depends on T003)
- [X] T011 [P] Replace `order.balance` with `order.balanceOrZero` in
...
- [X] T012 In `lib/features/sales/presentation/capture/capture_step.dart`:
...
- [X] T013 [P] In `lib/features/sales/presentation/capture/sale_line_row.dart`:
...
- [X] T014 [P] In `lib/features/sales/presentation/capture/sale_line_card.dart`:
      add `showWarehouse` (`bool`, default `true`); make its warehouse picker
      (`:192`) conditional — contracts/quote-capture-host.md §3
- [X] T015 In `lib/features/sales/presentation/capture/sale_line_layout.dart`:
...
- [X] T016 [P] Extend `test/widget/features/sales/sale_line_symmetry_test.dart`
...
- [X] T017 [P] Update the register's `CaptureStep` call site
...
- [X] T018 [P] In `lib/features/sales/presentation/capture/product_stock_cache.dart`,
...
- [X] T019 Extract the generic ~15 lines of
...
- [X] T020 [P] Create `lib/features/sales/presentation/sales_quote_write_scope.dart`:
...
- [X] T021 [P] Create `lib/features/sales/domain/entities/sales_quote_summary.dart`:
...
- [X] T022 Create `lib/features/sales/domain/repositories/sales_quote_repository.dart`:
...
- [X] T023 [P] Promote the five `anyOf: [string, num]` wire-value setters
      ...
      §3
- [X] T024 Create `lib/features/sales/data/sales_quote_repository_impl.dart`:
      ...
      on T022, T023)
- [X] T025 In `sales_quote_repository_impl.dart`, add `_toQuoteError`: a 422
...
- [X] T026 Create `lib/features/sales/presentation/quote_editing.dart`:
      ...
      T024)
- [X] T027 Create `lib/features/sales/presentation/quotes/quote_editor_controller.dart`:
...
- [X] T028 [P] In `lib/core/navigation/nav_destinations.dart`: add
      ...
      R6
- [X] T029 [P] Add l10n key `salesQuotesMenuTitle` ("Sales Quotes"/
      "Cotizaciones") to `lib/l10n/app_en.arb` and `app_es.arb`, in parity —
      used for both the nav label and the app-bar title
- [X] T030 Create `lib/features/sales/presentation/quotes/quote_screen.dart`:
...
- [X] T031 [P] Create `lib/features/sales/presentation/quotes/sales_quotes_list_controller.dart`:
...
- [X] T032 [P] Create `lib/features/sales/presentation/quotes/sales_quotes_list_screen.dart`:
      a skeleton screen (no columns/rows yet — added in US3) so the router has
      a builder target (depends on T031)
- [X] T033 In `lib/app/router/app_router.dart`: append a new last
...
- [X] T034 [P] Extend `test/widget/features/sales/pos_test_harness.dart`:
...
- [X] T035 Generalize `test/widget/features/sales/sale_editor_isolation_test.dart`
...
- [X] T037 [P] [US1] Widget test in
      ...
      folio (FR-022)
- [X] T038 [P] [US1] Widget test extension: confirming assigns a folio and
      makes the screen read-only; the primary action is **absent, not
      greyed** once confirmed (FR-021, FR-023)
- [X] T039 [P] [US1] Widget test in `quote_screen_test.dart`'s header group:
      ...
      (FR-020, FR-023)
- [X] T040 [P] [US1] Integration test in
...
- [X] T041 [US1] Create `lib/features/sales/presentation/quotes/quote_header_panel.dart`:
...
- [X] T042 [US1] In `quote_screen.dart`, wire `onContinue` to call
...
- [X] T043 [P] [US1] Add l10n keys: `salesQuoteConfirmAction`,
...
- [X] T044 [P] [US2] Widget test in `quote_screen_test.dart`'s convert group:
...
- [X] T045 [P] [US2] Extend `sales_quotes_flow_test.dart` (T040): confirm →
...
- [X] T046 [US2] In `quote_screen.dart`, add the Convert action: visible only
...
- [X] T047 [US2] In `quote_screen.dart`, wire a **working** Duplicate action
...
- [X] T048 [P] [US2] Add l10n keys `salesQuoteConvertAction` and
...
- [X] T052 [P] [US3] Widget test: reopening a draft resumes it editable with
...
- [X] T053 [P] [US3] Router test group in
...
- [X] T054 [US3] Flesh out `sales_quotes_list_screen.dart`: a
...
- [X] T055 [P] [US3] Create `SalesQuoteStatusChip` (built on
      `core/widgets/status_chip.dart`) plus a separate expired-marker widget
      — FR-024, FR-037
- [X] T056 [US3] In `quote_screen.dart`, add the Cancel action, visible only
      ...
      (research.md R5)
- [X] T057 [P] [US3] Add l10n keys: `salesQuotesColumnReference`,
...
- [X] T058 [P] [US4] Widget test: from `quote_screen.dart`'s customer band,
...
- [X] T059 [US4] **Needs no new code.** `CustomerBar`'s existing inline-create
...
- [X] T060 [P] [US5] Widget test: duplicating a quote in any state (draft,
...
- [X] T061 [P] [US5] Widget test: from an expired quote's failed-convert
      state (T044), the offered Duplicate action navigates to the new draft

### Implementation for User Story 5

- [X] T062 [US5] In `quote_screen.dart`, **widen** T047's Duplicate action
...
- [X] T063 [P] [US5] Confirm `salesQuoteDuplicateAction` (added by T048 for
...
- [X] T064 [P] Re-run the three-host isolation test (T035) after all five
      stories have landed; confirm it is still green
- [X] T065 [P] Confirm `git diff --stat main --
      ...
      either host
- [X] T066 [P] `flutter analyze` clean; full suite green, including T002's
      baseline re-run with unchanged assertions
- [X] T067 Run quickstart.md's full validation: the automated commands, then
      ...
      target deployment
- [X] T068 [P] Diff `lib/l10n/app_en.arb` and `app_es.arb` against their
      pre-feature state; confirm no key outside this feature's own changed
- [X] T069 Manually confirm each of SC-001 through SC-008 against the

...
void main() {
  group('fetchClampedPage (017-ui-consistency-filters FR-026)', () {
    ...
    test(
      'a page beyond the result set clamps to the last valid page instead '
      'of surfacing an empty view',
      () async {
        final calls = <int>[];
        final page = await fetchClampedPage<int>(
          pageIndex: 5,
    ...
    test(
      'a genuinely empty result (no matches at all) is left alone, not '
      'clamped',
      () async {
        final calls = <int>[];
        final page = await fetchClampedPage<int>(
    ...
    test(
      'when the computed last page equals the requested page, does not '

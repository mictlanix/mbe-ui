...
void main() {
  group('SatCatalogRepositoryImpl.listUnitsOfMeasurement (existing, unaffected)', () {
  ...
  group('SatCatalogRepositoryImpl.listProductServices (existing, unaffected)', () {
    test('200 maps to SatCatalogItems', () async {
      final repository = _repositoryWith(
        (options) async => ResponseBody.fromString(
          jsonEncode({
  ...
  group('SatCatalogRepositoryImpl.listPostalCodes (spec 012 research.md §5)', () {
    test('200 maps to SatCatalogItems', () async {
      final repository = _repositoryWith(
        (options) async => ResponseBody.fromString(
          jsonEncode({
  ...
  group('SatCatalogRepositoryImpl.listTaxRegimes (spec 012 research.md §5)', () {
    test('200 maps to SatCatalogItems', () async {
      final repository = _repositoryWith(
        (options) async => ResponseBody.fromString(
          jsonEncode({

...
Future<void> _pump(WidgetTester tester, AppError error, {Locale locale = const Locale('en')}) {
...
void main() {
  group('ErrorBanner localization (017-ui-consistency-filters, research §6)', () {
    testWidgets('renders the localized English message for each generic variant', (tester) async {
      ...
      expect(
        find.text('Something went wrong on the server. Please try again later.'),
        findsOneWidget,
      );
    ...
    testWidgets('renders the localized Spanish message for each generic variant', (tester) async {
      const es = Locale('es');
      ...
      expect(find.text('No se encontró el elemento solicitado.'), findsOneWidget);

      await _pump(tester, const AppError.server(), locale: es);
      expect(
        find.text('Ocurrió un error en el servidor. Inténtalo de nuevo más tarde.'),
        findsOneWidget,
      );

      await _pump(tester, const AppError.network(), locale: es);
      expect(
        find.text('No se pudo conectar con el servidor. Verifica tu conexión e inténtalo de nuevo.'),
        findsOneWidget,
      );
    ...
    testWidgets('ValidationError with field errors shows the server-provided messages, not the generic one', (
      tester,
    ) async {
      await _pump(
        tester,

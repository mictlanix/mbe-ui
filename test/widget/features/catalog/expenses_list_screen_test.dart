...
const _testExpenses = [
  Expense(expenseId: 1, name: 'Rent', comment: 'Monthly'),
  Expense(expenseId: 2, name: 'Fuel'),
];
...
void main() {
  ...
  testWidgets('shows name and comment for every expense', (tester) async {
  ...
  testWidgets('search box and pagination are present', (tester) async {
    ...
    expect(find.byKey(const Key('expenses_search_field')), findsOneWidget);
  ...
  testWidgets('the Create button is hidden without create privilege', (
    ...
    expect(find.byKey(const Key('new_expense_button')), findsNothing);
  });
  ...
  testWidgets('an empty result shows the empty state', (tester) async {
    await pumpScreen(tester, signedInAs: _fullAccessUser, expenses: const []);

    expect(find.byKey(const Key('expenses_table')), findsNothing);
  });
}
